...
def run(env, num_iterations, eta, d, num_tiles, tiling_size, \
    c, c_in_stepsize, critic_m, critic_lr, critic_stop_trs, actor_init_theta, actor_m, actor_lr, actor_stop_trs, actor_d, actor_num_tiles, actor_tiling_size, sampling):
    # initialize actor object to update policy parameters
    actor = DirectLinearMDPOActor(env, eta, c_in_stepsize, actor_init_theta, actor_d, actor_num_tiles, actor_tiling_size, actor_m, actor_lr, actor_stop_trs)
    if c_in_stepsize:
        actor.set_c(c)

    critic = DirectLFAACPGCritic(env, d, num_tiles, tiling_size, eta, c, critic_m, critic_lr, critic_stop_trs)
    ...
    for t in range(num_iterations):
        ...
        # get/save the critic loss
        critic_loss = critic.get_loss(Q, curr_policy, d, critic.theta)
        critic_loss_list.append(critic_loss)

        # get/save the critic grad notm
        critic_grad = critic.get_gradient(Q, curr_policy, d, critic.theta)
...
if __name__ == '__main__':
    ...
    if args.is_config == 1:
        try:
            ...
            yaml_file_directory = 'Configs/' + args.env + "_" + args.sampling + "_Direct_LinearACPG_" + str(args.d) + ".yaml"
    ...
    current_dir = os.path.join(os.path.abspath(os.getcwd()), "Results/Direct_LinearACPG_LFAACPG/"+ cfg['env'] + "_" +cfg['sampling'] + "/")
