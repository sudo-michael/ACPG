...
import argparse
import os
from ACPG.Actor import SoftmaxTabularACPGActor
...
def run(env, num_iterations, eta, init_theta, iht_size, num_tiles, tiling_size, c, mc, lrc):
    ...
    actor = SoftmaxTabularACPGActor(env, eta, init_theta, c)

    # initialize critic to estimate Q function. 
    #iht_size = env.state_space * env.action_space
    critic = SoftmaxLFAACPGCritic(env, iht_size, num_tiles, tiling_size, eta, c, mc, lrc)
    ...
    # Storing A function estimation error 
    diff_in_A = []
...
if __name__ == '__main__':
    ...
    current_dir = os.path.join(os.path.abspath(os.getcwd()), "Results/Softmax_TabularACPG_LFAACPG/")
